...
namespace TaxumoChatBot.Upe
{
    public static class ServicePrincipal
    {
        ...
        static string authority = Program.Configuration["UPE:ida:Authority"];
        ...
        static string clientId = Program.Configuration["UPE:ida:ClientId"];

        // The key that was created for the calling AAD app. For example: oCgdj3EYLfnR0p6iR3UvHFAfkn+zQB+0VqZT/6=
        static string clientSecret = Program.Configuration["UPE:ida:ClientSecret"];
        ...
        static string resource = Program.Configuration["UPE:ida:Resource"];

        public static async System.Threading.Tasks.Task<AuthenticationResult> GetAccessTokenAsync()
        {
            if (string.IsNullOrEmpty(authority) || string.IsNullOrEmpty(resource) ||
                string.IsNullOrEmpty(clientId) || string.IsNullOrEmpty(clientSecret))
            {
                return null;
            }

            return await GetS2SAccessTokenAsync(authority, resource, clientId, clientSecret);
        }

        ///<summary>
        /// Gets an application token used for service-to-service (S2S) API calls.
        ///</summary>
        static async System.Threading.Tasks.Task<AuthenticationResult> GetS2SAccessTokenAsync(string authority, string resource, string clientId, string clientSecret)
        {
